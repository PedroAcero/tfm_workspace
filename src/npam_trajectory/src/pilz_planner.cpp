...
bool parseG1Line(const std::string &line, geometry_msgs::msg::Pose &pose) {
   ...
   * cuaternión (I,J,K,W).
   * F ya no se interpreta, se toma un escalado del 100%
   */
...
std::vector<geometry_msgs::msg::Pose> gcode_reader(const std::string &filename,
  ...
  // Idéntica a cartesian_planner.cpp: retorna vector de Pose sin feedrate
...
buildPoseConstraints(const geometry_msgs::msg::Pose &pose,
   ...
   *
   * Pilz LIN no acepta poses directamente como computeCartesianPath,
   * sino que requiere goal_constraints en formato MoveIt genérico.
...
buildSequenceRequest(const std::vector<geometry_msgs::msg::Pose> &waypoints,
                     const std::string &group_name,
                     const std::string &end_effector_link,
                     const std::string &planning_frame, double velocity_scaling,
                     rclcpp::Logger logger) {
  /**
   ...
   *   - max_velocity_scaling_factor = 1.0: escalado al 100%
   *   - max_acceleration_scaling_factor = 1.0: escalado al 100%
   *   - blend_radius = 0.0: paso exacto por cada waypoint
   *
   * La velocidad real de ejecución la controlan los límites definidos en
   * pilz_cartesian_limits.yaml (max_trans_vel, max_trans_acc, etc.).
   * Modificar ese archivo es la forma de iterar sobre la velocidad
   * sin recompilar el nodo.
   */
  ...
  for (size_t i = 0; i < waypoints.size(); i++) {
    ...
    // Escalado al 100%: la velocidad la controla pilz_cartesian_limits.yaml
    ...
    item.req.goal_constraints.push_back(
        buildPoseConstraints(waypoints[i], end_effector_link, planning_frame));

    // blend_radius = 0.0: paso exacto por cada waypoint
    // Ver discusión en TFM: trade-off blend_radius vs. precisión geométrica
...
int main(int argc, char *argv[]) {
  ...
  // Igual que en cartesian_planner.cpp. MoveGroupInterface se usa
  // para consultas de estado y ejecución final de la trayectoria.
  // La planificación se delega al servidor Pilz vía MoveGroupSequence.
  // ============================================================
  ...
  // Idéntico a cartesian_planner.cpp: retorna vector de Pose.
  // El feedrate F del G-code se ignora en esta versión.
  // ============================================================
  ...
  //
  // Reemplaza los dos bloques de cartesian_planner.cpp:
  //   [Planificación OMPL → computeCartesianPath]
  //   [Parametrización temporal IPTP → applyTimeParameterization]
  //
  // Pilz unifica ambos pasos: interpola la línea recta cartesiana,
  // resuelve IK y calcula el perfil trapezoidal de velocidad
  // respetando los límites de pilz_cartesian_limits.yaml.
  //
  // La velocidad de ejecución se controla exclusivamente mediante
  // pilz_cartesian_limits.yaml. Para iterar sobre la velocidad,
  // modificar max_trans_vel en ese archivo y recompilar ur_moveit_config.
  // ============================================================
  moveit_msgs::msg::MotionSequenceRequest sequence_request =
      buildSequenceRequest(waypoints, PLANNING_GROUP, end_effector_link,
                           planning_frame, velocity_scaling, logger);
  ...
  if (!action_client->wait_for_action_server(std::chrono::seconds(10))) {
    RCLCPP_ERROR(logger,
                 ...
                 "cargado como pipeline en move_group.");
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
